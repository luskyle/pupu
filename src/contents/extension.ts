...
const ACTIONS_NOT_NEED_TRUST_DOMAIN = ["PUPU_EXTENSION_REQUEST_TRUST_DOMAIN"];
...
async function isOriginTrusted(origin: string, action: string): Promise<boolean> {
  ...
  return isHostnameTrusted(origin, trustedDomains);
}

window.addEventListener("message", async (event) => {
  const request: ExtensionExternalRequest<unknown> = event.data;

  if (request.type !== "request" || !request.action.startsWith("PUPU")) {
    return;
  }
