...
export async function DynamicWeibo(data: SyncData) {
  ...
  // 辅助函数：等待所有图片上传完成
  // 判断依据：①无"上传中"遮罩（兼容旧 class 及变体）；②所有图片缩略图已加载出真实图
  // （complete && naturalWidth>0 && src 为 http）；③发送按钮已可点击（平台视角的最终信号）。
  // 满足 ①+② 或 ③ 即认为上传完成，避免图片还没传完就点发送导致动态发布失败。
  function waitForUploadsToComplete(timeout = 120000): Promise<void> {
    return new Promise((resolve) => {
      const startTime = Date.now();

      const checkStatus = () => {
        // 上传中遮罩（兼容旧 class 与可能的变体）
        const loadingElements = document.querySelectorAll(
          '.Image_loading_1lfUB, [class*="Image_loading"], [class*="woo-pic-loading"]',
        );

        // 图片缩略图：每张图带"删除"按钮，其容器内的 img
        const deleteIcons = document.querySelectorAll('i[title="删除"]');
        const thumbImgs = Array.from(deleteIcons)
          .map((icon) => {
            const holder = icon.closest("li, div, span") || icon.parentElement;
            return holder?.querySelector("img");
          })
          .filter(Boolean) as HTMLImageElement[];
        const allThumbsLoaded =
          deleteIcons.length > 0 &&
          thumbImgs.length >= deleteIcons.length &&
          thumbImgs.every((im) => im.complete && im.naturalWidth > 0 && im.src.startsWith("http"));
  ...
  try {
    ...
    // 组合正文、tags(#tag# 是微博话题写法)；动态已去掉独立标题，title 为空时不再前置换行
