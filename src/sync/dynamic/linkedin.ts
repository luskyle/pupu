...
export async function DynamicLinkedin(data: SyncData) {
  ...
  try {
    ...
    logger.debug("triggerButton", triggerButton);
    if (!triggerButton) {
      logger.debug("未找到触发按钮");
      return;
    }
    triggerButton.click();
    ...
    if (mediaFiles.length > 0) {
      ...
      for (let i = 0; i < mediaFiles.length; i++) {
        if (i >= 8) {
          logger.debug("Linkedin 最多支持 8 张 ，跳过");
          break;
        }
    ...
    logger.error("LinkedIn 发布过程中出错:", error);
  }
}
