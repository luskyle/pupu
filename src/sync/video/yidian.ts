...
export async function VideoYidian(data: SyncData): Promise<void> {
  ...
  try {
    ...
    logger.debug("📝 视频数据:", { title: title?.substring(0, 50), contentLength: content?.length, hasVideo: !!video });
    ...
    logger.debug("✅ 一点号上传器类定义完成");

    const uploader = new YidianVideoUploader();
    logger.debug("✅ 一点号上传器实例创建完成");
    ...
    logger.debug("🎉 一点号视频发布流程完成");
    return;
  } catch (error) {
    logger.error("💥 一点号视频发布失败:", error);
