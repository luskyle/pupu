...
export async function VideoPinduoduo(data: SyncData): Promise<void> {
  ...
  try {
    ...
    logger.debug("✅ 拼多多上传器类定义完成");

    const uploader = new PinduoduoVideoUploader();
    logger.debug("✅ 拼多多上传器实例创建完成");
    ...
    logger.debug("🎉 拼多多视频发布流程完成");
    return;
  } catch (error) {
    logger.error("💥 拼多多视频发布失败:", error);
