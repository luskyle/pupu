  ...
  try {
    ...
    const DayuVideoUploader = class DayuVideoUploader {
      ...
      public async uploadHorizontalCover(coverData: any): Promise<void> {
        logger.debug("📐 开始处理横版封面...", coverData);

        if (!coverData || !coverData.url) {
          logger.debug("⚠️ 未提供横版封面图片");
          return;
        }
        ...
        try {
          ...
          if (coverData.coverFile) {
            ...
            logger.error("❌ 无效的封面数据");
            return;
          }

          logger.debug("📁 横版封面文件:", file.name, file.size, file.type);
            ...
            if (selector.includes(":contains")) {
              ...
              for (const elem of elements) {
                ...
                if (label && (label.textContent?.includes("视频封面") || label.textContent?.includes("封面"))) {
                  logger.debug(`✅ 通过标签文本找到横版封面上传区域: ${baseSelector}`);
                  uploadArea = elem;
                  break;
                }
              }
              ...
              const element = document.querySelector(selector);
              if (element && (element as HTMLElement).offsetParent !== null) {
                logger.debug(`✅ 找到横版封面上传区域: ${selector}`);
                uploadArea = element;
                break;
              }
            }
          ...
          if (!uploadArea) {
            ...
            for (const input of fileInputArray) {
              ...
              if (accept.includes("image") || accept.includes("jpg") || accept.includes("png")) {
                uploadArea = input;
                logger.debug("✅ 找到图片文件输入框作为横版封面上传区域");
                break;
              }
            }
          }
          ...
          if (!uploadArea) {
            logger.debug("❌ 未找到横版封面上传区域");
            return;
          }

          // 执行封面上传
          logger.debug("🚀 开始执行横版封面上传...");
          await this.performCoverUpload(uploadArea as HTMLElement, file, "horizontal");
          logger.debug("✅ 横版封面上传完成");
          return;
        } catch (error) {
          logger.error("横版封面上传失败:", error);
          return;
        }
      }
      ...
      public async uploadVerticalCover(coverData: any): Promise<void> {
        logger.debug("📱 开始处理竖版封面...", coverData);

        if (!coverData || !coverData.url) {
          logger.debug("⚠️ 未提供竖版封面图片");
          return;
        }
        ...
        try {
          ...
          if (coverData.verticalCoverFile) {
            ...
            logger.error("❌ 无效的竖版封面数据");
            return;
          }

          logger.debug("📁 竖版封面文件:", file.name, file.size, file.type);
          ...
          logger.debug("🔍 查找竖版封面上传区域...");

          let uploadArea: Element | null = null;

          // 方法1: 通过精确的层级结构查找
          logger.debug("🔍 方法1: 通过层级结构查找...");
          ...
          if (!uploadArea) {
            logger.debug("❌ 未找到竖版封面上传区域");
            return;
          }

          // 执行封面上传
          logger.debug("🚀 开始执行竖版封面上传...");
          await this.performCoverUpload(uploadArea as HTMLElement, file, "vertical");
          logger.debug("✅ 竖版封面上传完成");
          return;
        } catch (error) {
          logger.error("竖版封面上传失败:", error);
          return;
        }
      }
      ...
      public async uploadVideoTags(tags: string[]): Promise<void> {
        logger.debug("🏷️ 开始处理视频标签...", tags);

        if (!tags || tags.length === 0) {
          logger.debug("⚠️ 未提供视频标签");
          return;
        }
        ...
        try {
          ...
          if (tagContainer) {
            ...
            if (allInputDivs.length >= 1) {
              ...
              try {
                ...
                logger.debug("⚠️ Vue.js标准方式设置失败，使用DOM操作");
              }
            } else {
              logger.debug("⚠️ 未找到输入框，等待更长时间或使用备选方案");
              await this.sleep(3000);
              ...
              if (retryInputDivs.length >= 1) {
                ...
                logger.debug("✅ 延迟后找到输入框");
              } else {
                logger.debug("❌ 延迟后仍未找到输入框");
              }
            }
          }
            ...
            for (const label of Array.from(tagLabels)) {
              if (label.textContent?.includes("标签")) {
                ...
                if (fieldContent) {
                  ...
                  logger.debug("⚠️ 找到的元素不是INPUT或TEXTAREA，跳过");
                }
              }
            }
          }
          ...
          if (!tagInput) {
            logger.debug("🔍 方法5: 跳过可编辑元素查找（避免设置错误的DIV元素）");
            logger.debug("⚠️ 为避免设置错误的contenteditable元素，跳过此方法");
            // 注意: 用户明确指出contenteditable是错误的，所以跳过此方法
          }

          // 方法6: 查找Vue.js特有的元素（仅INPUT/TEXTAREA）
          if (!tagInput) {
            logger.debug("🔍 方法6: 查找Vue.js特有元素...");
          ...
          if (!tagInput && tagContainer) {
            logger.debug("🔍 方法7: 深度搜索标签容器内所有元素...");
            ...
            for (const child of Array.from(allChildren)) {
              ...
              if (tagName === "INPUT" || tagName === "TEXTAREA") {
                ...
                if (
                  ...
                  logger.debug("✅ 在容器内找到INPUT/TEXTAREA元素");
                  tagInput = elem as HTMLInputElement | HTMLTextAreaElement;
                  break;
                }
              }
          ...
          if (!tagInput) {
            logger.debug("⚠️ 未在预期位置找到标签输入框，尝试动态触发...");

            // 尝试点击标签容器看是否能触发输入框出现
            if (tagContainer) {
              logger.debug("🖱️ 点击标签容器尝试触发输入框...");
          ...
          // 格式化标签字符串
          logger.debug("🏷️ 格式化标签字符串:", tags.join(", "));
          const tagString = tags.join(", ");

          // 填写标签
          logger.debug("📝 填写标签:", tagString);
          logger.debug("📝 标签数组:", tags);

          // 检查是否有输入框可以填写
          if (tagInput) {
            logger.debug("✅ 找到标签输入框，开始填写...", {
              tagName: tagInput.tagName,
            ...
            if (tagInput.tagName === "INPUT" || tagInput.tagName === "TEXTAREA") {
              logger.debug("✅ 使用INPUT/TEXTAREA方式填写标签");

              // 重新查找所有标签输入框（确保获取最新的）
              const tagContainer = document.querySelector(".article-write_video-tags.form-control");
              const allInputDivs = tagContainer ? tagContainer.querySelectorAll('div > input[type="text"]') : [];
              logger.debug(`✅ 找到 ${allInputDivs.length} 个标签输入框，将分配 ${tags.length} 个标签`);

              if (allInputDivs.length === 0) {
                logger.debug("❌ 未找到任何输入框，可能Vue.js组件未正确激活");
                return;
              }

              // ✅ 正确的实现：逐个输入标签，每次按回车键触发下一个输入框
              let filledCount = 0;
              logger.debug(`📋 将处理 ${tags.length} 个标签（逐个输入，按回车键触发下一个）`);
              ...
              for (let i = 0; i < tags.length; i++) {
                ...
                try {
                  ...
                  if (!currentInput) {
                    logger.debug(`❌ 未找到第 ${i + 1} 个输入框`);
                    continue;
                  }

                  // 确保输入框可见
                  if (currentInput.offsetParent === null) {
                    logger.debug(`❌ 第 ${i + 1} 个输入框不可见`);
                    continue;
                  }

                  logger.debug(`🎯 找到第 ${i + 1} 个输入框，开始输入`);
                  ...
                  logger.debug(`📊 输入完成，当前值: "${currentInput.value}"`);
                  ...
                  if (i < tags.length - 1) {
                    ...
                    // 等待下一个输入框出现
                    logger.debug("⏳ 等待下一个输入框出现...");
                    await this.sleep(1500);

                    // 验证下一个输入框是否出现
                    const nextInputs = tagContainer.querySelectorAll('div > input[type="text"]');
                    logger.debug(`📊 按回车后输入框数量: ${nextInputs.length}`);

                    if (nextInputs.length > currentInputs.length) {
                      logger.debug(`✅ 成功触发第 ${i + 2} 个输入框`);
                    } else {
                      logger.debug("⚠️ 可能未成功触发下一个输入框，但继续处理");
                    }
                  } else {
                    logger.debug("✅ 最后一个标签，处理完成");
                  }

                  // 7. 验证当前标签是否成功
                  if (currentInput.value === tag) {
                    filledCount++;
                    logger.debug(`✅ 第 ${i + 1} 个标签输入成功 ✅`);
                  } else {
                    logger.debug(`⚠️ 第 ${i + 1} 个标签值不匹配，尝试强制设置`);
                    // 强制设置
                    currentInput.focus();
                    await this.sleep(200);
                    ...
                    if (currentInput.value === tag) {
                      filledCount++;
                      logger.debug(`✅ 第 ${i + 1} 个标签强制设置成功 ✅`);
                    }
                  }
                } catch (e) {
                  logger.error(`❌ 第 ${i + 1} 个标签处理出错:`, e);
                }

                // 在处理下一个标签前等待
                if (i < tags.length - 1) {
                  logger.debug("⏳ 等待500ms后处理下一个标签...");
                  await this.sleep(500);
                }
              }
              ...
              for (
                ...
                if (remainingInput) {
                  ...
                  logger.debug(`🧹 清空第 ${i + 1} 个剩余输入框`);
                }
              }

              logger.debug(`✅ 标签填写完成，成功填写 ${filledCount}/${tags.length} 个标签`);

              if (filledCount === 0) {
                logger.debug("❌ 所有标签输入都失败");
                return;
              }
            ...
            // 错误的方法 - 用户明确指出这是错误的
            logger.debug("❌ 发现错误的contentEditable方法，这会导致错误的HTML结构");
            logger.debug("💡 应该使用Vue.js框架提供的多个INPUT输入框，而不是contenteditable div");
            logger.debug("⚠️ 跳过错误的contentEditable处理，使用备选方案");

            // 标记为需要手动处理
            logger.debug("ℹ️ 标签处理需要手动完成（使用框架方法）");
            return;
          }

          // 如果仍然没有找到输入框，尝试使用其他方法
          logger.debug("⚠️ 仍未找到可编辑的标签输入框");
          logger.debug("💡 尝试将标签写入剪贴板，方便手动粘贴...");

          // 尝试复制到剪贴板
          try {
            await navigator.clipboard.writeText(tagString);
            logger.debug("✅ 标签已复制到剪贴板: ", tagString);
            logger.debug("💡 提示: 请手动粘贴到标签输入框中");
          } catch (_err) {
            logger.debug("⚠️ 剪贴板复制失败，请手动输入标签");
          }

          // 标记为成功（因为可能是需要手动输入的区域）
          logger.debug("ℹ️ 标签处理标记为完成（可能需要手动输入）");
        } catch (error) {
          logger.error("视频标签处理失败:", error);
          return;
        }
      }
      ...
      private async performCoverUpload(
        ...
        try {
          logger.debug(`🚀 执行${coverType}封面上传...`);
          logger.debug("📍 上传区域信息:", {
          ...
          if (uploadArea.tagName === "INPUT" && (uploadArea as HTMLInputElement).type === "file") {
            logger.debug(`✅ 找到${coverType}封面文件输入框`);
            ...
            logger.debug(`✅ ${coverType}封面文件已设置到输入框`);

            // 等待文件处理和弹框出现
            logger.debug("⏳ 等待文件处理和弹框出现...");
            await this.sleep(3000);
          ...
          if (fileInput) {
            logger.debug(`✅ 在${coverType}封面上传区域内找到文件输入框`);
            logger.debug("📍 文件输入框信息:", {
          ...
          logger.debug(`❌ 无法处理${coverType}封面上传`);
          return;
        } catch (error) {
          logger.error(`${coverType}封面上传失败:`, error);
          return;
        }
      }
      ...
      private async handleImageCropDialog(): Promise<void> {
        try {
          logger.debug("🖼️ 开始处理图片裁剪弹窗...");
          logger.debug("⏳ 等待5秒确保文件处理完成...");
          await new Promise((resolve) => setTimeout(resolve, 5000));

          logger.debug("🔍 开始扫描页面弹框...");
          ...
          if (foundDialog) {
            logger.debug("✅ 发现弹框，开始处理保存操作");
            await this.executeConfirmStrategy();
            logger.debug("✅ 图片裁剪弹窗保存完成");
          } else {
            logger.debug("⚠️ 未发现裁剪弹窗，可能不需要裁剪或已直接应用");
          }

          logger.debug("⚠️ 图片裁剪弹窗保存未完成，但继续后续流程");
        } catch (error) {
          logger.error("💥 视频封面保存过程出错:", error);
        }
      }
      ...
      private async executeConfirmStrategy(): Promise<void> {
        ...
        try {
            ...
            for (const button of buttons) {
              ...
              if (btn.offsetParent !== null) {
                ...
                if (text.includes("保存") || text.includes("确定") || text.includes("完成") || text.includes("确认")) {
                  logger.debug(`✅ 找到保存按钮: "${text}" | ${selector}`);
                  btn.click();
                  await this.sleep(1000);
                  return;
                }
          ...
          logger.debug("❌ 未找到保存按钮");
          return;
        } catch (error) {
          logger.error("保存策略执行失败:", error);
          return;
        }
      }
        ...
        try {
          ...
          for (const selector of sourceSelectors) {
            ...
            if (input && input.offsetParent !== null) {
              logger.debug("✅ 找到信息来源选项:", selector);
              sourceInput = input;
              break;
            }
          }
          ...
          if (!sourceInput) {
            logger.debug("⚠️ 未找到信息来源选项，可能页面结构变化");
            return;
          }

          // 检查是否已经选中
          if (sourceInput.checked) {
            logger.debug('✅ 信息来源已经选择为"无需标注"');
            return;
          }

          // 点击选择
          logger.debug('🖱️ 点击选择"无需标注"...');
          sourceInput.click();
          await this.sleep(500);
          ...
          // 验证选择是否成功
          if (sourceInput.checked) {
            logger.debug('✅ 信息来源选择成功: "无需标注"');
            return;
          }
          logger.debug("⚠️ 信息来源选择可能失败，但继续...");
          return; // 标记为成功以继续后续流程
        } catch (error) {
          logger.error("❌ 信息来源选择失败:", error);
          return;
        }
      }
    };

    logger.debug("✅ 大鱼号上传器类定义完成");

    const uploader = new DayuVideoUploader();
    logger.debug("✅ 大鱼号上传器实例创建完成");
    ...
    // 步骤4: 上传横版封面
    if (cover) {
      logger.debug("🖼️ 开始上传横版封面...");
      await uploader.uploadHorizontalCover(cover);
    } else {
      logger.debug("⚠️ 未提供横版封面图片，跳过横版封面上传");
    }
    ...
    if (verticalCoverData) {
      logger.debug("📱 开始上传竖版封面...");
      await uploader.uploadVerticalCover(verticalCoverData);
    } else {
      logger.debug("⚠️ 未提供竖版封面图片，跳过竖版封面上传");
    }

    // 步骤6: 处理视频标签
    if (tags && tags.length > 0) {
      logger.debug("🏷️ 开始处理视频标签...");
      await uploader.uploadVideoTags(tags);
    } else {
      logger.debug("⚠️ 未提供视频标签，跳过标签处理");
    }

    // 步骤7: 选择信息来源（默认：无需标注）
    logger.debug("📋 开始设置信息来源...");
    await uploader.selectVideoSource();

    logger.debug("🎉 大鱼号视频发布流程完成");
    return;
  } catch (error) {
    logger.error("💥 大鱼号视频发布失败:", error);
