...
export const ChejiahaoVideoUploader = class ChejiahaoVideoUploader {
  ...
  private async getUploadToken(): Promise<string> {
    try {
      logger.debug("🔑 获取车家号上传凭证...");

      // 尝试从已有的全局变量获取
      if ((window as any).browser_0_?.params?.callback) {
        logger.debug("✅ 从全局变量获取上传凭证");
      ...
      if (response.ok) {
        ...
        logger.debug("✅ 获取上传凭证成功:", data);
        this.uploadToken = data.token || "";
        return this.uploadToken;
      }

      logger.debug("⚠️ 无法获取上传凭证，使用默认值");
      return "";
    } catch (error) {
  ...
  private async initAHVPUploader(): Promise<boolean> {
    try {
      logger.debug("🚀 初始化AHVP上传器...");

      // 等待AHVP系统加载
      if (!(window as any).AHVP) {
        logger.debug("🔄 等待AHVP系统加载...");
        let attempts = 0;
      ...
      if (!AHVP) {
        logger.error("❌ AHVP系统未加载");
        return false;
      }

      logger.debug("✅ AHVP系统已加载");
      ...
      if (this.uploader) {
        ...
        logger.debug("✅ AHVP上传器创建成功");
        return true;
      }

      logger.error("❌ AHVP上传器创建失败");
      return false;
    } catch (error) {
      logger.error("❌ 初始化AHVP上传器失败:", error);
      return false;
    }
  }
    ...
    try {
      ...
      for (const input of fileInputs) {
        ...
        if (id.includes("browser_0") || !accept.includes("image") || accept.includes("video")) {
          targetInput = input;
          logger.debug(`✅ 找到目标输入框: ${id || "unnamed"}`);
          break;
        }
      }
      ...
      if (!targetInput) {
        ...
        targetInput.id = "pupu_chejiahao_video_input";
      ...
      if (this.uploader && typeof this.uploader.addFile === "function") {
        logger.debug("🔄 通过AHVP上传器添加文件");
        try {
          this.uploader.addFile(file);
          logger.debug("✅ 文件已添加到AHVP上传器");
        } catch (_error) {
          logger.debug("⚠️ AHVP添加文件失败，使用标准方式");
        }
      }
      ...
      if (!uploadStarted) {
        logger.debug("⚠️ 未检测到明确的上传开始信号，但文件已设置");
      }

      logger.debug("🎉 视频文件上传流程完成");
      return;
    } catch (error) {
    ...
    try {
      ...
      for (const button of buttons) {
        ...
        if (textContent.includes("发布")) {
          logger.debug("✅ 找到发布按钮:", textContent);
          button.click();
          await this.sleep(2000);
          logger.debug("✅ 发布按钮点击成功");
          return;
        }
      }
  ...
  try {
    ...
    const ChejiahaoVideoUploaderInline = class ChejiahaoVideoUploader {
      ...
      public async fillTitle(title: string): Promise<void> {
        try {
          ...
          if (titleElement) {
            logger.debug("✅ 找到标题输入框: #title");
            logger.debug("  - placeholder:", titleElement.placeholder);
            logger.debug("  - 可见性:", titleElement.offsetParent !== null ? "可见" : "隐藏");
            ...
            try {
              ...
              logger.debug("⚠️ 标题值不匹配，继续...");
            } catch (e) {
              logger.error("设置标题值时出错:", e);
            }
          } else {
            logger.debug("❌ 未找到#title输入框");
          }

          logger.debug("❌ 标题填写失败，但继续流程");
          return;
        } catch (error) {
          logger.error("填写标题失败:", error);
          return;
      ...
      public async checkOriginalAndFirst(): Promise<void> {
        try {
          ...
          if (firstCheckbox) {
            ...
            logger.debug("❌ 未找到首发复选框");
          }

          return;
        } catch (error) {
          logger.error("❌ 勾选原创和首发失败:", error);
          return;
        }
      }
      ...
      private async createImageFile(fileData: NonNullable<VideoData["cover"]>): Promise<File | null> {
        try {
          if (!fileData.url) {
            logger.debug("车家号封面数据缺少 URL，跳过上传");
            return null;
          }

          if (fileData.type && !fileData.type.includes("image/")) {
            logger.debug("车家号封面不是图片类型，跳过上传:", fileData.type);
            return null;
          }
      ...
      public async uploadCover(coverData: NonNullable<VideoData["cover"]>): Promise<boolean> {
        ...
        try {
          logger.debug("🖼️ 开始上传车家号封面:", coverData);

          const editCoverButton = this.findCoverEditButton();
          logger.debug("cover edit button -->", editCoverButton);
          if (!editCoverButton) {
            logger.debug("未找到车家号封面编辑入口，跳过封面上传");
            return false;
          }
          ...
          logger.debug("cover file input -->", fileInput);
          if (!fileInput) {
            logger.debug("未找到车家号封面上传输入框，跳过封面上传");
          ...
          logger.warn("车家号封面上传失败，继续发布流程:", error);
      ...
      private async waitForVideoUploadComplete(
        ...
        while (Date.now() - startedAt < timeout) {
          ...
          if (this.hasUploadFailureText(statusText)) {
            logger.warn("车家号视频上传失败，跳过自动发布:", statusText.substring(0, 200));
            return false;
          }
          ...
          if (hasSuccessMarker) {
            if (successWasAbsentAfterCurrentUpload) {
              logger.debug("✅ 车家号视频上传已确认完成");
              return true;
            }

            if (!loggedStaleSuccessMarker) {
              logger.warn("车家号检测到本次上传前已存在的完成标记，等待当前上传产生新的完成状态");
      ...
      private async confirmVideoUploadIfNeeded(
        ...
        if (!waitForCompletion) {
          logger.debug("车家号手动发布流程：视频上传已触发，不等待上传完成");
          return false;
        }
      ...
      public async publishIfAutoEnabled(autoPublish: boolean, videoUploaded: boolean): Promise<void> {
        ...
        if (!videoUploaded) {
          logger.warn("车家号自动发布已跳过：视频未确认上传成功");
          return;
        }

        if (this.usedHardcodedUploadFallback) {
          logger.warn("车家号自动发布已跳过：本次上传使用了硬编码 AHVP fallback 参数");
          return;
        }
        ...
        if (!publishButton) {
          logger.debug("未找到车家号发布按钮");
          return;
        }

        publishButton.dispatchEvent(new Event("click", { bubbles: true }));
        logger.debug("✅ 车家号发布按钮点击成功");
      }
        ...
        try {
          ...
          const captureCurrentUploadBaseline = () => {
            ...
            if (currentUploadSuccessWasPresentBeforeTrigger) {
              logger.debug("⚠️ 检测到旧的上传完成标记，本次上传需要等待新的完成状态");
            }
          };
          ...
          const confirmCurrentUploadIfNeeded = async (timeout = 120000, uploadWasSeen = false) => {
            if (!currentUploadTriggered) {
              logger.warn("车家号忽略上传状态：当前视频文件尚未触发选择或change事件");
              return false;
            }
          ...
          // 首先检查上传区域状态
          logger.debug("🔍 检查上传区域状态...");
          const browserElement = document.querySelector("#browser_0");
          if (browserElement) {
            logger.debug("✅ 找到browser_0元素");

            // 检查browser_0内的a标签
            const uploadLink = browserElement.querySelector("a");
            if (uploadLink) {
              logger.debug("✅ 找到browser_0内的a标签");
              logger.debug("  - a标签文本:", uploadLink.textContent?.substring(0, 50));
              logger.debug("  - a标签href:", uploadLink.href);
              logger.debug("  - a标签class:", uploadLink.className);
              ...
              if (
                ...
                logger.debug("⚠️ 检测到上传状态，但当前视频尚未触发上传，继续设置本次文件");
              }

              if (linkText.includes("上传失败") || linkText.includes("重新上传")) {
                logger.debug("❌ 检测到上传失败状态");
                return false;
              }

              if (this.hasUploadSuccessText(linkText)) {
                logger.debug("⚠️ 检测到旧的上传完成状态，继续设置本次文件");
              }
            } else {
              logger.debug("❌ 未找到browser_0内的a标签");
            }

            logger.debug("  - browser_0子元素数量:", browserElement.children.length);
            logger.debug("  - browser_0内容:", browserElement.textContent?.substring(0, 100));

            // 如果a标签消失或内容变化，可能上传已经开始
            if (!uploadLink || browserElement.children.length === 0) {
              logger.debug("⚠️ a标签消失或browser_0内容变化，检查上传状态...");

              const progressElements = document.querySelectorAll(
                '[class*="progress"], [class*="upload"], .ant-progress',
              );
              if (progressElements.length > 0) {
                logger.debug("⚠️ 检测到既有上传进度元素，继续设置本次文件");
              }
            }
          } else {
            logger.debug("❌ 未找到browser_0元素");
          }

          // 检查车家号的上传系统
          logger.debug("🔍 检查车家号上传系统...");
          logger.debug("  - window.AHVP:", typeof (window as any).AHVP);
          logger.debug("  - window.muploader:", typeof (window as any).muploader);
          logger.debug("  - window.browser_0_:", typeof (window as any).browser_0_);

          // 诊断AHVP加载状态
          logger.debug("🔍 诊断AHVP加载状态...");
          logger.debug("  - window.AHVP:", typeof (window as any).AHVP);
          logger.debug("  - window.muploader:", typeof (window as any).muploader);
          logger.debug("  - window.browser_0_:", typeof (window as any).browser_0_);
          ...
          if (browserElementForUpload) {
            ...
            if (uploadLink) {
              const linkText = uploadLink.textContent || "";
              logger.debug("📋 点击后的a标签文本:", linkText.substring(0, 100));
            ...
            existingFileInputs.forEach((existingInput, index) => {
              logger.debug(`📋 找到现有文件输入框 ${index + 1}:`, existingInput.accept);
            ...
            if (!fileSetSuccess) {
              ...
              if (uploadLinkForClick) {
                logger.debug("🖱️ 尝试模拟用户点击browser_0的a标签...");
                ...
                newFileInput.id = `pupu_after_click_${Date.now()}`;
                ...
                // 触发事件
                logger.debug("🔄 在用户激活后触发文件change事件...");
            ...
            if (finalLink) {
              ...
              logger.debug("📋 最终的a标签文本:", finalText.substring(0, 100));
              ...
              if (
                ...
                logger.debug("✅ 文件上传触发成功！检测到实际上传状态");
                return await confirmCurrentUploadIfNeeded(120000, true);
              }
          ...
          // 如果直接操作失败，再尝试AHVP系统
          logger.debug("⏳ 直接操作无效，尝试等待AHVP系统...");
          ...
          if (!AHVP) {
            logger.error("❌ AHVP系统未加载，但检测到AHVP脚本已存在");
            logger.error("💡 CSP策略禁止了eval()，无法强制重新初始化");
            logger.debug("🔄 转为检测现有上传状态...");

            // 既然无法使用AHVP，检查是否已经有其他上传机制在工作
            // 详细检查页面状态
            logger.debug("🔍 详细检查页面状态...");
            ...
            if (browserElement) {
              ...
              if (uploadLink) {
                logger.debug("✅ 找到browser_0内的a标签");
                const linkText = uploadLink.textContent || "";
                logger.debug("  - a标签文本:", linkText.substring(0, 100));
                ...
                if (
                  ...
                  logger.debug("✅ a标签显示上传状态，上传正在进行中");
                  return await confirmCurrentUploadIfNeeded(120000, true);
                }

                if (linkText.includes("上传失败") || linkText.includes("重新上传")) {
                  logger.debug("❌ a标签显示上传失败");
                  return false;
                }

                if (this.hasUploadSuccessText(linkText)) {
                  logger.debug("🎉 a标签显示上传完成，等待确认是否属于本次上传");
                  return await confirmCurrentUploadIfNeeded(120000, false);
                }
              } else {
                logger.debug("❌ 未找到browser_0内的a标签");
                ...
                if (browserText.includes("上传中") || browserText.includes("已上传")) {
                  logger.debug("✅ browser_0显示上传状态（a标签可能已被替换）");
                  return await confirmCurrentUploadIfNeeded(120000, true);
                }
              }
            } else {
            ...
            fileInputs.forEach((input, index) => {
              if (input.files && input.files.length > 0) {
                logger.debug(`✅ 文件输入框${index}已有文件:`, input.files[0].name);
                filesFound = true;
              }
            });

            if (filesFound) {
              logger.debug("✅ 检测到文件已设置到输入框，上传可能已开始");
            ...
            (window as any).XMLHttpRequest = () => {
              ...
              xhr.open = function (method: string, url: string | URL, ...args: any[]) {
                if (url?.toString().includes("upload")) {
                  logger.debug("✅ 检测到上传XHR:", method, url);
                  uploadActive = true;
                }
            ...
            if (uploadActive) {
              logger.debug("✅ 检测到上传活动");
              return await confirmCurrentUploadIfNeeded(120000, true);
            }

            // 5. 检查页面文本内容中的上传状态
            logger.debug("🔄 检查页面文本中的上传状态...");
            ...
            for (const indicator of uploadStatusIndicators) {
              if (bodyText.includes(indicator)) {
                logger.debug(`✅ 在页面文本中找到上传状态指示: "${indicator}"`);
                uploadDetected = true;
              }
            }

            if (uploadDetected) {
              logger.debug("✅ 检测到页面显示上传状态，上传正在进行中");

              // 进一步检查具体的上传状态
              if (bodyText.includes("上传中")) {
                logger.debug("📊 状态: 上传进行中");
                return await confirmCurrentUploadIfNeeded(120000, true);
              }
              if (bodyText.includes("上传失败") || bodyText.includes("重新上传")) {
                logger.debug("❌ 状态: 上传失败");
                return false;
              }
              if (this.hasUploadSuccessText(bodyText)) {
                logger.debug("🎉 状态: 上传完成，等待确认是否属于本次上传");
                return await confirmCurrentUploadIfNeeded(120000, false);
              }
            ...
            logger.debug("❌ 无法检测到明确的上传活动");
            logger.debug("🔧 建议: AHVP系统可能需要手动触发或页面刷新后重试");
            return false;
          }
          ...
          // 验证AHVP功能
          logger.debug("🔍 验证AHVP功能...");
          logger.debug("  - AHVP.newUploaderManager:", typeof AHVP.newUploaderManager);
          logger.debug("  - AHVP.UPLOADER_EVENT:", !!AHVP.UPLOADER_EVENT);
          ...
          // 添加文件到manager
          logger.debug("📁 添加文件到manager...");
          manager.addItem(fileItem);
          logger.debug("✅ 文件添加到manager");
          ...
          if (!waitForCompletion) {
            logger.debug("车家号手动发布流程：AHVP fallback 上传已启动，不等待上传完成");
            return false;
          }
          ...
          return new Promise<boolean>((resolve) => {
            ...
            setTimeout(() => {
              if (!uploadCompleted) {
                logger.warn("⏰ 车家号视频上传超时，未视为成功，跳过自动发布");
                finish(false);
              }
            }, 120000); // 2分钟超时
          });
    ...
    if (cover) {
      await uploader.uploadCover(cover).catch((error) => {
        logger.warn("车家号封面上传异常，继续发布流程:", error);
        return false;
      });
    }
    ...
    logger.debug("🎉 车家号视频发布流程完成");
    return;
  } catch (error) {
    logger.error("💥 车家号视频发布失败:", error);
    logger.error("错误详情:", error.stack);
    return;
  }
}
