...
export async function VideoYiche(data: SyncData): Promise<void> {
  ...
  try {
    ...
    logger.debug("✅ 易车上传器类定义完成");

    const uploader = new YicheVideoUploader();
    logger.debug("✅ 易车上传器实例创建完成");
    ...
    logger.debug("🎉 易车视频发布流程完成");
    return;
  } catch (error) {
    logger.error("💥 易车视频发布失败:", error);
