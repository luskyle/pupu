...
export async function VideoSohu(data: SyncData): Promise<void> {
  ...
  try {
    ...
    logger.debug("✅ 搜狐号上传器类定义完成");

    const uploader = new SohuVideoUploader();
    logger.debug("✅ 搜狐号上传器实例创建完成");
    ...
    logger.debug("🎉 搜狐号视频发布流程完成");
    return;
  } catch (error) {
    logger.error("💥 搜狐号视频发布失败:", error);
