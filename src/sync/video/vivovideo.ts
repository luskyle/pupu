...
export async function VideoVivoVideo(data: SyncData): Promise<void> {
  ...
  try {
    ...
    logger.debug("✅ vivo视频上传器类定义完成");

    const uploader = new VivoVideoUploader();
    logger.debug("✅ vivo视频上传器实例创建完成");
    ...
    logger.debug("🎉 vivo视频发布流程完成");
    return;
  } catch (error) {
    logger.error("💥 vivo视频发布失败:", error);
