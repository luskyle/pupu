...
export async function ArticleWeibo(data: SyncData) {
  ...
  async function getAccountId() {
    // uid 提取：覆盖多种格式（$CONFIG['uid'] / $CONFIG["uid"] / $CONFIG.uid / JSON "uid": N），
    // 优先从当前已渲染页面的脚本提取，避免 fetch 拿到未登录/空的页面。
    const uidPatterns = [
      /\$CONFIG\['uid'\]\s*=\s*['"]?(\d+)['"]?\s*;/,
      /\$CONFIG\["uid"\]\s*=\s*['"]?(\d+)['"]?\s*;/,
      /\$CONFIG\.uid\s*=\s*['"]?(\d+)['"]?\s*;/,
    ...
    const uidSnips = Array.from(document.scripts)
      .map((s) => s.textContent || "")
